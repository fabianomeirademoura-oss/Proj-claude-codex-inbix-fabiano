# CLAUDE.md — Instruções de entrada para o Claude Code
...
trabalho, `../painel-claude`. O Codex
trabalha no mesmo projeto, com as instruções em [AGENTS.md](AGENTS.md).
...
- **Trabalhe em `../painel-claude`**, a sua pasta de trabalho. Ela já assina os commits
  como `Claude Code (agente)`. A pasta `painel-horizonte/` é a `main` do coordenador, e
  `../painel-codex` é a do outro agente: não edite nenhuma das duas.
- Cada tarefa começa numa branch nova:
  `git fetch origin && git switch -c claude/<tarefa> origin/main`.
- **Todo commit termina com a linha `Agente: Claude Code`.** Sem ela, o commit é
  recusado.
...
- Arquivos `.ps1` são gravados em **UTF-8 com BOM**. Sem o BOM, o Windows
  PowerShell 5.1 estraga os acentos.
...
as verificações executadas, as limitações e os impactos para o Codex) e marque-o
